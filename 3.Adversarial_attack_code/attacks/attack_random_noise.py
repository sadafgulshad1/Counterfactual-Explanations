...
class AttackRandom:
    ...
    def run(self, model, input, target, batch_idx=0):
        ...
        #input_adv = input.cpu().numpy() + eps*torch.sign(torch.normal(torch.FloatTensor([0.0]), torch.FloatTensor([1.0])))
        input_adv = input.cpu().numpy() + eps*torch.sign(torch.randn(input.size()))
