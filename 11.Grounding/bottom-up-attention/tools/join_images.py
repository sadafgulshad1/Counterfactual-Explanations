...
def demo(image_name,image_no,image_name_AT,image_no_AT):
    im = cv2.imread(os.path.join("/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/Pytorch_Code/transfer_learn/pytorch-cw2/adv_bb/",image_name))
    im_AT = cv2.imread(os.path.join("/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/Pytorch_Code/transfer_learn/pytorch-cw2/clean_bb/",image_name_AT))
    ...
    plt.suptitle("Ground Truth class: "+(correct_cls[int(image_no)])+", Predicted class: "+(wrong_cls[int(image_no)]),fontsize=2)
    plt.savefig('/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/Pytorch_Code/transfer_learn/pytorch-cw2/Merged/Merged{}'.format(image_no), dpi = 1500) 
    #plt.savefig('/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/Pytorch_Code/transfer_learn/Analysis/Merged/merged{}'.format(image_no), dpi = 1500) 
...
file_names=[sorted(glob.glob( os.path.join("/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/Pytorch_Code/transfer_learn/pytorch-cw2/adv_bb",'*.jpg') ),key=lambda x:float(re.findall("([0-9]+?)\.jpg",x)[0]))]
...
##################
file_names_AT=[sorted(glob.glob( os.path.join("/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/Pytorch_Code/transfer_learn/pytorch-cw2/clean_bb",'*.jpg') ),key=lambda x:float(re.findall("([0-9]+?)\.jpg",x)[0]))]
...
for j in range (len(image_no_AT)):
    ...
    demo(image_name[j],image_no[j], image_name_AT[j],image_no_AT[j])
