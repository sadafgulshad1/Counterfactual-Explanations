...
att_cls=[]
image_numb=[]
with open('/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/LAD_experiments/Analysis/complete_data/ground_clean_attr_50.txt') as inputfile:
...
inputfile.closed
"""
att_names = [word.replace('color:is black','black') for word in att_names]
att_names = [word.replace('color:is white','white') for word in att_names]
att_names = [word.replace('color:is blue','blue') for word in att_names]
att_names = [word.replace('color:is brown','brown') for word in att_names]
att_names = [word.replace('color:is gray','gray') for word in att_names]
att_names = [word.replace('color:is orange','orange') for word in att_names]
att_names = [word.replace('color:is red','red') for word in att_names]
att_names = [word.replace('color:is green','green') for word in att_names]
att_names = [word.replace('color:is yellow','yellow') for word in att_names]
att_names = [word.replace('appearance: is furry','furry') for word in att_names]
att_cls = [word.replace('appearance: has soft skin','skin') for word in att_names]
att_cls = [word.replace('appearance: has tough skin','skin') for word in att_names]
att_cls = [word.replace('appearance: has feathers','feathers') for word in att_names]
att_cls = [word.replace('appearance: has whiskers','whiskers') for word in att_names]
att_cls = [word.replace('marking: has patches','patch') for word in att_names]
att_cls = [word.replace('marking: has spots','spots') for word in att_names]
att_cls = [word.replace('marking: has stripes','stripes') for word in att_names]
att_names = [word.replace('size: is small (compared to pigs)','small') for word in att_names]
att_names = [word.replace('size: is big (compared to pigs)','big') for word in att_names]
"""
...
att_index_adv=[]
att_cls_adv=[]
with open('/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/LAD_experiments/Analysis/complete_data/ground_adv_attr_50.txt') as inputfile:
    for row in csv.reader(inputfile):
        att_names_adv.append(row[0])
...
def demo(image_name,image_no,net):
    ...
    im = cv2.imread(os.path.join("/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/LAD_experiments/Analysis/complete_data/adv_images/",image_name))
        ...
        if attr_conf[i] > attr_thresh:
            ...
            if attributes[attr[i]+1] in att_unique:
                if classes[objects[i]+1] in att_unique:
                    ...
                    plt.gca().text(bbox[0], bbox[1]-2,'%s' % (cls),bbox=dict(facecolor='blue', alpha=0,linewidth=0.2),fontsize=1.5, color='red')
                    #att = attributes[attr[i]+1]
                    #cls = classes[objects[i]+1]
                    #count_box=count_box+1
                    #plt.gca().add_patch(plt.Rectangle((bbox[0], bbox[1]),bbox[2] - bbox[0],bbox[3] - bbox[1], fill=False,edgecolor='red', linewidth=0.3, alpha=0.5))
                    #plt.gca().text(bbox[0], bbox[1]-2,'%s' % (cls),bbox=dict(facecolor='blue', alpha=0,linewidth=0.2),fontsize=1.5, color='red')
                    #plt.gca().add_patch(plt.Rectangle((bbox[0], bbox[1]),bbox[2] - bbox[0],bbox[3] - bbox[1], fill=False,edgecolor='black', linewidth=0.3, alpha=0.5))
                    #plt.gca().text(bbox[2]-10, bbox[3],'%s' % (att),bbox=dict(facecolor='blue', alpha=0,linewidth=0.2),fontsize=1.5, color='red')



    plt.savefig('/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/LAD_experiments/Analysis/adv_bb/Adv_bb{}.jpg'.format(image_no), dpi = 1500)
...
file_names=[sorted(glob.glob("/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/LAD_experiments/Analysis/complete_data/adv_images/*.jpg"),key=lambda x:float(re.findall("([0-9]+?)\.jpg",x)[0]))]
...
for i in range (len(file_names[0])):
    ...
    image_no.append(int(file_names[0][i].split("/")[9].split("Adv")[1].split(".jpg")[0]))
