...
with open('/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/AWA_data/Animals_with_Attributes2/discriminative_attr_class_all.txt') as inputfile:
    for row in csv.reader(inputfile):
        correct_cls.append(row[0].split(" ")[0].split(".")[1])
        wrong_cls.append(row[0].split(" ")[1].split(".")[1])
inputfile.closed
#Loading attributes
att_names=[]
att_index=[]
att_cls=[]
image_numb=[]
with open('/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/AWA_data/Animals_with_Attributes2/ground_clean_attr_25.txt') as inputfile:
...
att_index_adv=[]
att_cls_adv=[]
with open('/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/AWA_data/Animals_with_Attributes2/ground_adv_attr_25.txt') as inputfile:
    for row in csv.reader(inputfile):
...
def demo(image_name,image_no,image_index,net):
    ...
    im = cv2.imread(os.path.join("/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/AWA_data/Animals_with_Attributes2/clean_images",image_name))
    ...
    for i in range(len(keep_boxes)):
        ...
        if attr_conf[i] > attr_thresh:
            ...
            if attributes[attr[i]+1] in att_unique:
                ...
                plt.gca().text(bbox[0], bbox[1]+30,'%s' % (cls),bbox=dict(facecolor='blue', alpha=0,linewidth=0.2),fontsize=2, color=colors[i])
    ...
    plt.savefig('/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/AWA_data/Animals_with_Attributes2/clean_images_2/clean_bb{}.jpg'.format(image_no), dpi = 1500)
...
file_names=[sorted(glob.glob("/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/AWA_data/Animals_with_Attributes2/clean_images/*.jpg"),key=lambda x:float(re.findall("([0-9]+?)\.jpg",x)[0]))]
...
for i in range (len(file_names[0])):
    ...
    image_no.append(int(file_names[0][i].split("/")[8].split("clean")[1].split(".jpg")[0]))
