...
def demo(image_name,image_no,image_name_AT,image_no_AT,image_index):
    ...
    im = cv2.imread(os.path.join("/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/AWA_data/Animals_with_Attributes2/adv_bb1",image_name))
    im_AT = cv2.imread(os.path.join("/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/AWA_data/Animals_with_Attributes2/clean_bb1",image_name_AT))
    ...
    plt.savefig('/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/AWA_data/Animals_with_Attributes2/Merged_1/Merged{}'.format(image_no), dpi = 1500)
    plt.close()
...
file_names=[sorted(glob.glob( os.path.join("/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/AWA_data/Animals_with_Attributes2/adv_bb1",'*.jpg') ),key=lambda x:float(re.findall("([0-9]+?)\.jpg",x)[0]))]
...
for i in range (len(file_names[0])):
    ...
    image_no.append(int(file_names[0][i].split("/")[8].split("bb")[1].split(".jpg")[0]))
...
##################
file_names_AT=[sorted(glob.glob( os.path.join("/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/AWA_data/Animals_with_Attributes2/clean_bb1",'*.jpg') ),key=lambda x:float(re.findall("([0-9]+?)\.jpg",x)[0]))]
...
for i in range (len(file_names_AT[0])):
    image_name_AT.append(file_names_AT[0][i].split("/")[8])
    image_no_AT.append(int(file_names_AT[0][i].split("/")[8].split("bb")[1].split(".jpg")[0]))
