...
"""
file_names=[glob.glob("/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/Pytorch_Code/transfer_learn/Analysis/CUB_adv/*.jpg")]
image_name=[]
image_no=[]
for i in range (len(file_names[0])):
    image_name.append(file_names[0][i].split("/")[6])
    image_no.append(i)
...
im = [cv2.imread(file) for file in glob.glob("/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/Pytorch_Code/transfer_learn/Analysis/CUB_adv/*.jpg")]
im=np.asarray(im)
print (im.shape)
for j in range (im.shape[0]):
    ...
    scores, boxes, attr_scores, rel_scores = im_detect(net, im1)
    ...
    ############################
    att_unique=np.unique(att_names[image_no[j]*scale:(image_no[j]*scale+scale)])
    print (att_unique)
    att_unique_adv=np.unique(att_names_adv[image_no[j]*scale:(image_no[j]*scale+scale)])
    cls_unique=np.unique(att_cls[image_no[j]*scale:(image_no[j]*scale+scale)])
    print (cls_unique)
    cls_unique_adv=np.unique(att_cls_adv[image_no[j]*scale:(image_no[j]*scale+scale)])
    ...
    for i in range(len(keep_boxes)):
        ...
        if attr_conf[i] > attr_thresh:
            for k in range (len(att_unique_adv)):
                for l in range (len(cls_unique_adv)):
                    if attributes[attr[i]+1]==att_unique_adv[k]:
                        if classes[objects[i]+1] == cls_unique_adv[l]:
            ...
            plt.axis("off")
            plt.savefig('/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/Pytorch_Code/transfer_learn/Analysis/adv_bb/adv_bb{}_scale_s.jpg'.format(image_no[j])) 
            
            #plt.savefig('/media/sadaf/e4da0f25-29be-4c9e-a432-3193ff5f5baf/Code/Pytorch_Code/transfer_learn/Analysis/clean_bb/clean_bb{}_scale_s.jpg'.format(image_no[j])) 
           
        #print (cls)
#print ("att_names clean")
#print (att_names[3957*scale:(3957*scale+scale)])
#print ("att_names adv")
#print (att_names_adv[3957*scale:(3957*scale+scale)])
