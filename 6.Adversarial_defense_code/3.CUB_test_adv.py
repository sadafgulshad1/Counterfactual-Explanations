...
dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=16,
              ...
              for x in ['output_CUB']}
dataset_sizes = {x: len(image_datasets[x]) for x in ['output_CUB']}
print (dataset_sizes)
class_names = image_datasets['output_CUB'].classes
...
inputs, classes = next(iter(dataloaders['output_CUB']))
print (inputs.shape)
...
images_incorrect=[]
predicted_list=[]
for data in dataloaders['output_CUB']:
    # get the inputs
    images, labels = data
    if use_gpu:
