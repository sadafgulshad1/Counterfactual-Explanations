...
data_transforms = {
    'output_adv_test':transforms.Compose([
...
image_datasets = {x: datasets.ImageFolder(os.path.join(data_dir, x),
                                          data_transforms[x])
                  for x in ['output_adv_test']}
dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=16,
                                             shuffle=False, num_workers=4)
              for x in ['output_adv_test']}
dataset_sizes = {x: len(image_datasets[x]) for x in ['output_adv_test']}
print (dataset_sizes)
class_names = image_datasets['output_adv_test'].classes
...
inputs, classes = next(iter(dataloaders['output_adv_test']))
print (inputs.shape)
...
model=torch.load("adv_train_learn_CUB")    
model = model.cuda()
#model = torch.nn.DataParallel(model).cuda() 
checkpoint = torch.load("adv_train_learn_CUB.pth")        
...
images_incorrect=[]
predicted_list=[]
for data in dataloaders['output_adv_test']:
    # get the inputs
    images, labels = data
    if use_gpu:
...
with open ("adv_incorrect_index.txt","w") as f:
    np.savetxt(f,zip(incorrect_list,predicted_incorrect_list),fmt='%d')
with open ("adv_correct_index.txt","w") as f:
